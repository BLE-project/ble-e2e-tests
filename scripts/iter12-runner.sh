#!/bin/bash
# iter12-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Iter11 verifies Iter3-A fix landed in BiometricEnrollModal.tsx (BackHandler
# consumer added). No runner-side change vs iter10; APK re-install at flow-start
# picks up the freshly built release/app-release.apk.
...
OUT_DIR="$REPO_ROOT/test-results/iter12-2026-05-03"
mkdir -p "$OUT_DIR"
...
START_TIME=$(date +%s)
echo "iter12-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
