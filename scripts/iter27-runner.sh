#!/bin/bash
# iter27-runner.sh — iter18 base + Sprint13a-f deliverables landed
#
# Inherits iter18 base (pm grant pre-launch fix from Iter18-A) plus
# the consolidated Sprint13a-f changes:
#   - CC-001 BE chain live (cashback PRs #37 #38, core-registry #27)
#   - CC-001 FE wiring (consumer-mobile PR #60 — useFirstCardWatcher)
#   - TC-CR-001 Maestro flow (e2e-tests PR #9 — registration-first-scan.yaml)
#   - Iter27 sales-agent assertion widening (e2e-tests PR #10 —
#     navigation.yaml + requests.yaml: Dashboard -> Panoramica)
#
# Expected outcome vs iter26 baseline (1/15):
#   - sales-agent/login: PASS (unchanged from iter15+)
#   - sales-agent/navigation: was FAIL (Dashboard timeout), expect PASS
#   - sales-agent/requests: was FAIL (Dashboard timeout), expect PASS
#   - consumer/login: PASS (post-CC-001 wiring landed)
#   - other 11 flows: same as iter26 (no per-flow change)
#
# Iter18-A reminder: cold-install BLE permission dialogs (Android 12+
# BLUETOOTH_SCAN, BLUETOOTH_CONNECT, POST_NOTIFICATIONS) are pre-granted
# via `adb shell pm grant` AFTER reinstall_app, BEFORE launching maestro.
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[iter27-runner] another runner already holds $LOCK_FILE — refusing." >&2
    exit 2
  fi
else
  if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n "$OTHER_PID" ] && kill -0 "$OTHER_PID" 2>/dev/null; then
      echo "[iter27-runner] another runner (pid $OTHER_PID) holds $LOCK_FILE — refusing." >&2
      exit 2
    fi
  fi
  echo $$ > "$LOCK_FILE"
...
OUT_DIR="$REPO_ROOT/test-results/iter27-2026-05-04"
mkdir -p "$OUT_DIR"
...
run_flow() {
  ...
  if [ "$rc" -eq 124 ] || [ "$rc" -eq 137 ]; then
    echo "" >>"$log_file"
    echo "[iter27-runner] timeout fired after ${FLOW_TIMEOUT_SEC}s (rc=$rc)" >>"$log_file"
    reset_maestro_agent
    return 1
  fi
...
START_TIME=$(date +%s)
echo "iter27-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
