#!/bin/bash
# iter26-runner.sh — iter18 base + pm grant pre-launch (Iter18-A fix)
#
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    echo "[iter26-runner] another runner already holds $LOCK_FILE — refusing." >&2
    exit 2
  fi
else
  if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n "$OTHER_PID" ] && kill -0 "$OTHER_PID" 2>/dev/null; then
      echo "[iter26-runner] another runner (pid $OTHER_PID) holds $LOCK_FILE — refusing." >&2
      exit 2
    fi
  fi
  echo $$ > "$LOCK_FILE"
...
OUT_DIR="$REPO_ROOT/test-results/iter26-2026-05-04"
mkdir -p "$OUT_DIR"
...
run_flow() {
  ...
  if [ "$rc" -eq 124 ] || [ "$rc" -eq 137 ]; then
    echo "" >>"$log_file"
    echo "[iter26-runner] timeout fired after ${FLOW_TIMEOUT_SEC}s (rc=$rc)" >>"$log_file"
    reset_maestro_agent
    return 1
  fi
...
START_TIME=$(date +%s)
echo "iter26-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
