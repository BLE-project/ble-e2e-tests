#!/bin/bash
# iter8-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Mirrors iter6-runner.sh methodology so iter8 results are directly comparable.
# Reinstalls APK before login (clean SecureStore); force-stop between flows.
...
OUT_DIR="$REPO_ROOT/test-results/iter8-2026-05-03"
mkdir -p "$OUT_DIR"
...
START_TIME=$(date +%s)
echo "iter8-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
