#!/bin/bash
# iter9-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Mirrors iter8-runner.sh with 2 mitigations for Iter8-A (Maestro JVM native-OOM):
#   (1) sleep between flows extended 1s -> 3s (give SentinelOne EDR + JVM heap time to release)
#   (2) run_flow() retry regex extended to also catch JVM native-OOM at C2-compile time
...
OUT_DIR="$REPO_ROOT/test-results/iter9-2026-05-03"
mkdir -p "$OUT_DIR"
...
# Iter9 mitigation (2): extend retry regex to also recognise JVM native-OOM
# at C2 compile time (Iter8-A class) in addition to the existing
# "Unable to launch app" ADB device flake.
...
START_TIME=$(date +%s)
echo "iter9-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
echo "Device: $DEVICE" | tee -a "$OUT_DIR/SUMMARY.txt"
...
for entry in "${APPS[@]}"; do
  ...
  for flow_name in "${FLOW_ORDER[@]}"; do
    ...
    # Iter9 mitigation (1): 1s -> 3s — give SentinelOne agent + JVM heap
    # time to release between back-to-back Maestro CLI launches.
    sleep 3
  done
done
