#!/bin/bash
# iter7-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Mirrors iter6-runner.sh methodology so iter7 results are directly comparable.
...
OUT_DIR="$REPO_ROOT/test-results/iter7-2026-05-03"
mkdir -p "$OUT_DIR"
...
START_TIME=$(date +%s)
echo "iter7-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
