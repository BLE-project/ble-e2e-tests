#!/bin/bash
# iter10-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Mirrors iter9-runner.sh — mitigations from iter9 (Iter8-A class JVM native-OOM)
# remain in place; no behavioural change vs iter9 (regression-watchdog mode).
...
OUT_DIR="$REPO_ROOT/test-results/iter10-2026-05-03"
mkdir -p "$OUT_DIR"
...
# Inherited from iter9: retry regex covers ADB launch flakes + JVM native-OOM
# at C2 compile time (Iter8-A class).
...
START_TIME=$(date +%s)
echo "iter10-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
