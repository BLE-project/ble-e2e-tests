#!/bin/bash
# iter11-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
...
OUT_DIR="$REPO_ROOT/test-results/iter11-2026-05-03"
mkdir -p "$OUT_DIR"
...
START_TIME=$(date +%s)
echo "iter11-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
