#!/bin/bash
# iter13-runner.sh — focused 15-flow Maestro runner (login/navigation/logout x 5 apps)
# Iter13 verifies the YAML pivot fix landed in iter11 ledger:
# sales-agent-mobile/{navigation,logout}.yaml use `tapOn id: enroll-never`
# instead of `pressKey: Back` to opt out of biometric enroll modal.
# This sidesteps the Expo Router native-stack back-handling boundary that
# made the Option-A app-code BackHandler fix architecturally inert.
# iter12 was killed mid-flight (4/15 logs, merchant-mobile-login 0 bytes).
...
OUT_DIR="$REPO_ROOT/test-results/iter13-2026-05-03"
mkdir -p "$OUT_DIR"
...
START_TIME=$(date +%s)
echo "iter13-runner started $(date -u +%FT%TZ)" | tee "$OUT_DIR/SUMMARY.txt"
