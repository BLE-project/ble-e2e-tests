...
echo "==> ensure fake FCM service-account JSON exists (notification-service bind-mounts it)"
# notification-service mounts ./secrets/fake-fcm-sa.json → /run/secrets/fake-fcm-sa.json.
# That file is gitignored and only generated by the Makefile `fcm-sa` hook — which this
# script bypasses by calling `docker compose up` directly. On a fresh checkout the source
# path is missing, so Docker bind-mounts create an empty DIRECTORY at the target; then
# FcmPushSender fails to parse it ("Is a directory"), disables FCM, and every beacon push
# returns PUSH/DISABLED → 0 messages:send → beacon-push-fcm.spec.ts red. Generating it here
# (idempotent: skips when already present) makes the FCM stub path deterministic. #124.
( cd "$COMPOSE_DIR" && bash scripts/gen-fake-fcm-sa.sh )
